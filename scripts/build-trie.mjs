#!/usr/bin/env node
// Build a compact radix-trie from en-words.txt and emit a JS module (ESM)
// containing precompiled arrays and a single label blob for direct loading.
//
// Output: en-words-trie.js with exports:
//   - TRIE_DATA: {
//       LBL: string,
//       nodeFirstEdge: number[],
//       nodeEdgeCount: number[],
//       edgeLabelStart: number[],
//       edgeLabelLen: number[],
//       edgeTo: number[],
//       termBits: number[]  // bytes (0..255)
//     }
//   - TRIE_METADATA: { builtAt: string, generatorVersion: string, wordCount: number, nodeCount: number, edgeCount: number }
//
// Usage:
...
function compressRadix(node) {
  // Returns { e: boolean, edges: Array<{ label: string, to: node }> }
  const visit = (n) => {
    ...
    // Sort edges by first char for deterministic order (and faster scan)
...
async function writeBinary(outPath, enc) {
  ...
  // Validate label lengths fit in uint16
  let maxLabelLen = 0;
  ...
  // helper to copy typed arrays quickly
